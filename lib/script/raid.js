...
function generateRaidComments( aXHR, aAppendTo ) {
    HTMLString = "";

    HTMLString += "<div id=\"raidcomments\">";

    HTMLString += "<h3>" + L("Comments") + " (" + aXHR.comments.length + ")</h3>";

    HTMLString += "<ul>";
    $.each(aXHR.comments, function (index, comment) {
      var commentDate = formatDateTimeStringUTC(comment.InsertDate.split(" ")[0], comment.InsertDate.split(" ")[1]);
      HTMLString += "<li>" + commentDate + " " + comment.UserName + "<br>" + comment.Comment + "</li>";
    });
    HTMLString += "</ul>";

    HTMLString += "<textarea></textarea>";
    HTMLString += "<button id=\"saveCommentButton\" class=\"\">" + L("SendComment") + "</button>";

    HTMLString += "</div>";

    $("#"+aAppendTo).append(HTMLString);

    $("#saveCommentButton").button({ icons: { secondary: "ui-icon-disk" }})
      .click( function() { saveRaidComment(aXHR, $("#raidcomments textarea").val()); } );
}

function saveRaidComment( aXHR, sMessage) {
    var Parameters = {
        id         : aXHR.raidId,
        message    : sMessage,
    ...
    asyncQuery( "raid_comment", Parameters, generateRaid );
...
function generateRaid( aXHR )
{
    ...
    generateRaidComments( aXHR, "raiddetail" );

    loadRaidPanel( aXHR.show, RaidId );
    $("#raiddetail").show();

    $("#body").css("height", $("#body").height() + $("#raidcomments").height())
}
